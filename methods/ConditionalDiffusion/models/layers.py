        ...
        for i in range(n_blocks):
            for j in range(n_stages):
                setattr(
                    ...
                    conv3x3(
                        features, features, stride=1, bias=False, spec_norm=spec_norm
                    ),
                )
...
class MSFBlock(nn.Module):
    def __init__(self, in_planes, features, spec_norm=False):
        ...
        for i in range(len(in_planes)):
            self.convs.append(
                conv3x3(
                    in_planes[i], features, stride=1, bias=True, spec_norm=spec_norm
                )
            )
...
class CondRefineBlock(nn.Module):
    def __init__(
        ...
        self.crp = CondCRPBlock(
            features, 2, num_classes, normalizer, act, spec_norm=spec_norm
        )
